(function(root) {
  ...
  if (typeof self !== 'undefined' && typeof self.postMessage === 'function' &&
    ...
    var MPL_SETUP = [
      ...
      '    try:',
      '        # draw() SYNCHRONOUSLY, not the draw_idle() handle_json just',
      '        # scheduled: with no event loop here, that idle turn never comes,',
      '        # so refresh_all() would ship whatever is already in the buffer.',
      '        # Same fix, same reason, as the forced first frame above.',
      '        #',
      '        # Only a RESIZE actually needs it, which is why zoom, pan and home',
      '        # looked fine: they leave the buffer the same shape, so the stale',
      '        # frame still covers the canvas and the change merely appears one',
      '        # event late. A resize REALLOCATES the buffer, so the stale frame',
      '        # is gone and there is nothing to draw -- the figure DISAPPEARS,',
      '        # which is what dragging the corner did.',
      '        _m.canvas.draw()',
      '        _m.refresh_all()',
      '    except Exception:',
      '        pass',
